...
void cMainGame::Update()
{
	...
	cMatrix matRC = matRX * matRY; // 카메라 앵글

	m_vLookAt = m_vPosition;										// 현재 보는 곳
	m_vEye = cVector3(0.0f, m_fCameraDistance, -m_fCameraDistance); // 카메라 위치
	m_vEye = cVector3::TransformCoord(m_vEye, matRC);				// 카메라 회전
	m_vEye = m_vEye + m_vPosition;									// 카메라가 따라다님
...
void cMainGame::SetGrid()
{
	...
	// 글자 출력 위치
	m_vAxisXTextPosition = cVector3(fMax, 0, 0);  // x축
	m_vAxisZTextPosition = cVector3(0, 0, fMax);  // z축
}

void cMainGame::DrawGrid()
{
	// 고정을 하므로 world가 필요없음(rotate X, transform X)
	cMatrix mat = m_matView * m_matProj * m_matViewport;

	for (size_t i = 0; i < m_vecLineVertex.size(); i += 2) // 선을 그릴꺼라 2개씩 증가
	{
